...
from vcftophylogenetic import __version__ as vcftophylogenetic_VERSION

@click.group()
@click.version_option(prog_name='vcftophylogenetic', version=vcftophylogenetic_VERSION)
def main():
    """This is the vcftophylogenetic CLI"""
    pass
...
@click.command()
@click.option('--host', default='0.0.0.0', help='IP address to bind the vcftophylogenetic server to', show_default=True)
@click.option('--port', default='8080', help='Port number to bind the vcftophylogenetic server to', show_default=True)
@click.option('--infer-config', is_flag=True, help='If set: infer a basic configuration from the provided VCF and GFF/GFF3 files and do not use an existing `vcftophylogenetic.config.yml`')
...
def start(host: str, port: str, infer_config: bool, save_config): 
    from vcftophylogenetic.server import create_app
    ...
    if infer_config:
        ...
        click.secho('The following VCF files have been found. Please choose the one you want to visualize with vcftophylogenetic:', fg='yellow')
        i = 0
        for _vcf_file in vcf_files:
            click.secho('['+str(i)+'] '+os.path.basename(_vcf_file), fg='yellow')
            i = i + 1
        ...
        click.secho('The following GFF/GFF3 files have been found. Please choose the one you want to visualize with vcftophylogenetic:', fg='yellow')
        i = 0
    ...
    click.secho('Starting vcftophylogenetic server...', fg='green', bold=True)
    ...
    click.secho('vcftophylogenetic should be available under the following URL: '+str(url), fg='green', bold=True)
